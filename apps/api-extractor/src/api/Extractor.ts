...
export class Extractor {
  ...
  public static invoke(extractorConfig: ExtractorConfig, options?: IExtractorInvokeOptions): ExtractorResult {
    ...
    if (extractorConfig.apiReportEnabled) {
      const actualApiReportPathWithoutExtension: string = extractorConfig.reportTempFilePath.replace(
        /\.api\.md$/,
        ''
      );
      ...
      const actualApiReportContentMap: Map<string, string> =
        ApiReportGenerator.generateReviewFileContent(collector);

      // Compare it against the expected file
      for (const [modulePath, actualApiReportContent] of actualApiReportContentMap) {
        const actualEntryPointApiReportPath: string = `${actualApiReportPathWithoutExtension}${
          modulePath ? '.' : ''
        }${modulePath}.api.md`;
