...
import { WorkingPackage, IWorkingPackageEntryPoint } from './WorkingPackage';
...
export class Collector {
  ...
  public constructor(options: ICollectorOptions) {
    ...
    const entryPointSourceFile: ts.SourceFile | undefined = options.program.getSourceFile(
      this.extractorConfig.mainEntryPointFilePath.filePath
    );
  ...
  public analyze(): void {
    ...
    for (const entryPoint of this.workingPackage.entryPoints) {
      ...
      for (const exportedAstEntity of exportedAstEntities) {
        this._createEntityForIndirectReferences(exportedAstEntity, entryPoint, alreadySeenAstSymbols);
  ...
  private _createEntityForIndirectReferences(
    ...
    if (astEntity instanceof AstNamespaceImport) {
      ...
      for (const exportedEntity of astModuleExportInfo.exportedLocalEntities.values()) {
        ...
        const entity: CollectorEntity = this._createCollectorEntity(exportedEntity, entryPoint, undefined);
        entity.addAstNamespaceImports(astEntity);

        this._createEntityForIndirectReferences(exportedEntity, entryPoint, alreadySeenAstEntities);
      }
    }
  }
  ...
  private _makeUniqueNames(entryPoint: IWorkingPackageEntryPoint): void {
    ...
    for (const entity of entities) {
      for (const exportName of entity.exportNames) {
        if (exportName === 'default' && entity.nameForEmit && !usedNames.has(entity.nameForEmit)) {
          usedNames.add(entity.nameForEmit);
        } else if (usedNames.has(exportName)) {
